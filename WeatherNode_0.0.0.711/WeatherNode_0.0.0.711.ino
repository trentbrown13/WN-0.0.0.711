...
void lcdDisplayTemps2()
{
  ...
  Sensors.requestTemperatures();
  #ifdef Danube
     Serial.println("In LcdPrint2, #ifdef Danube");
     //tubTempF = getTubTemp;
...
void publishReadings(void)
{
  ...
  publishNTP();
  publishHeap();
 // #ifndef Liv_Patio //FIXME, WHY IS THIS NECESSARY?????????? - char[32] to small
...
void publishDallasTemps(void)
{
 ...
 // char outTmpCString[6];
  ...
  client.publish(Topic, outTempFString);
  
   // Publish Tub Temperature 
#ifdef Danube
  strcpy(Topic, Client);
